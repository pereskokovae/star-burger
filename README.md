...
```
## Быстрое обновление на сервере (Деплой)

На сервере находится bash-скрипт, который позволяет быстро обновить проект. Чтобы им воспользоваться, введите данную команду из `домашней директории`.
```sh
...
## Рабочая версия кода находится по [ссылке](https://devstarburger.ru/)
